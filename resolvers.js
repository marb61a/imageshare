...
const createToken = (user, secret, expiresIn) => {
  ...
  return jwt.sign(
    { username, email }, secret, { expiresIn }
  )
}
...
module.exports = {
    ...
    searchPosts: async(_, { searchTerm }, {Post}) => {
      if(searchTerm) {
        const searchResults = await Post.find(
          // Perform a text search for the value of searchTerm
          { $text: { $search: searchTerm }},

          // Assign a text score to searchTerm to provide the best match
          { score: { $meta: "textScore" } }

          // Then sort the results according to that text score
        )
        .sort({
          score: { $meta: "textScore" },
          likes: "desc"
  ...
  Mutation: {
    ...
    unlikePost: async(_, { postId, username }, { Post, User } ) => {
      // Find a post and subtract 1 from its likes value
      const post = await Post.findOneAndUpdate(
        { _id: postId},
        { $inc: { likes: -1 } },
        { new: true }
      ...
      /*
        Find User, remove the id of a post from its favorites array, 
        this will be populated as Posts
      */
      const user = await User.findOneAndUpdate(
        { username },
